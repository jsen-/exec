...
pub fn wait_timeout(pid: Pid, dur: Duration)
                    -> io::Result<Option<ExitStatus>> {
    INIT.call_once(State::init);
    unsafe {
        (*STATE).wait_timeout(pid.0, dur)
    }
}
...
impl State {
    ...
    fn wait_timeout(&self, pid: u32, dur: Duration)
                       -> io::Result<Option<ExitStatus>> {
...
pub fn wait(pid: Pid) -> io::Result<ExitStatus> {
    ...
    match unsafe { libc::waitpid(pid.0 as i32, &mut status, 0) } {
        n if n < 0 => return Err(io::Error::last_os_error()),
        n => {
            assert_eq!(n, pid.0 as i32);
